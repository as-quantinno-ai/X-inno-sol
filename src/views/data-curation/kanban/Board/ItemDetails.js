...
const ItemDetails = () => {
    ...
    const { columns, profiles, items, selectedItem, userStory } = kanban;
    ...
    const changeSlectedItem = () => {
        if (selectedItem !== false) {
            setOpen(true);
        }
    };
    useEffect(() => changeSlectedItem(), [selectedItem]);

    if (selectedItem !== false) {
        selectedData = items.filter((item) => item.transformerid === selectedItem)[0];
    }
