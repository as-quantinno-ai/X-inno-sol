...
const PresetColorBox = ({ color, presetColor, setPresetColor, handleColorClicks }) => (
    <Grid item>
        <Avatar
            variant="rounded"
            color="inherit"
...
PresetColorBox.propTypes = {
    ...
    handleColorClicks: PropTypes.func
};

const ThemePalette = ({ handleColorClick }) => {
    const theme = useTheme();

    const [presetColor, setPresetColor] = useState("");

    const handleColorClicks = (colorId) => {
        setPresetColor(colorId?.id);
            ...
            <Grid item container spacing={2} alignItems="center">
                {colorOptions.map((color, index) => (
                    <PresetColorBox
                        ...
                        handleColorClicks={handleColorClicks}
                    />
                ))}
            </Grid>
        </SubCard>
